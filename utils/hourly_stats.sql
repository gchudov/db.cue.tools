...
INSERT INTO stats_drives (SELECT substring(drivename from '([^ ]*) ') as label, count(DISTINCT userid) as cnt FROM submissions WHERE drivename IS NOT NULL GROUP BY label ORDER BY cnt DESC LIMIT 100);
