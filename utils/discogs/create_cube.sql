...
BEGIN
    ...
    IF count <= dim THEN
        FOR i IN 1..count LOOP
            selected[i] = offsets[i];
        END LOOP;
    ELSE
        FOR i IN 1..count LOOP
            dest = (dim * (i-1) / count) + 1;
            selected[dest] = selected[dest] + offsets[i];
        END LOOP;
    END IF;
...
CREATE INDEX toc_duration_index ON toc USING GIST (create_cube_from_toc(duration));
