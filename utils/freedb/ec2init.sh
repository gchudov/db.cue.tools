...
yum -y install php-cli php-xml s3cmd mercurial gcc make
yum -y upgrade
...
s3cmd --no-progress get s3://private.cuetools.net/freedb-complete-`date +%Y%m01`.tar.bz2 - | tar vxjO 2>&1 | ./freedb
