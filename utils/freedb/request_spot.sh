...
$EC2_HOME/bin/ec2-request-spot-instances -region "us-east-1" $AMIID --group "quick-start-1" --iam-profile $IROLE --key ec2 --instance-count 1 --price $PRICE --type one-time --instance-type m1.medium --user-data "$UDATA"
else
cat <<EOF
$UDATA
EOF
